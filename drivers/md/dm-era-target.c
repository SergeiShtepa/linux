...
static int era_ctr(struct dm_target *ti, unsigned argc, char **argv)
{
	...
	r = dm_get_device(ti, argv[0], FMODE_READ | FMODE_WRITE | FMODE_EXCL, &era->metadata_dev);
	if (r) {
	...
	r = dm_get_device(ti, argv[1], FMODE_READ | FMODE_WRITE | FMODE_EXCL, &era->origin_dev);
	if (r) {
